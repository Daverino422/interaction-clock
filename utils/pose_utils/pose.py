...
class Pose():
    ...
    def predict_pose(self):
        ...
        right_shoulder_ankle = self.is_point_in_keypoints("right_shoulder") and self.is_point_in_keypoints(
            "right_ankle")
        ...
        foot = self.get_available_point(
            ["left_foot_index", "right_foot_index", "left_heel", "right_heel", "left_ankle", "right_ankle"])
    ...
    def estimate(self) -> None:
        ...
        while self.video_reader.is_opened():
            ...
            retStr = self.test_timer.update()
            image = self.draw.actionText(image, retStr)
            ...
            if results.pose_landmarks is not None:
                ...
                self.test_timer.unpause()
            else:
                image = self.draw.pose_text(image, "No current pose/action, pausing timer")
...
class Pushup(Pose):
    ...
    def measure(self) -> None:
        ...
        while self.video_reader.is_opened():
            ...
            # progress bar
            image = cv2.rectangle(image, (0, self.height // 8 - 10),
        ...
        while self.video_reader.is_opened():
            ...
            if results.pose_landmarks is not None:
                ...
                h_m_s_ms_time = self.timer.convert_time(
                    time)  # convert Seconds to Hour : Minute : Second : Milli-Second format
