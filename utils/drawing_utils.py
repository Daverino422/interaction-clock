...
class Draw():
    ...
    def __init__(self, width, height) -> None:
        self.width = width
        self.height = height
        self.font = ImageFont.truetype('../data/fonts/arial.ttf', self.height // 15, encoding="unic")
    ...
    def actionText(self, image, action):
        """ Write text in image """
        pil_img = Image.fromarray(image)
        pil_draw = ImageDraw.Draw(pil_img)
        text_width, _ = pil_draw.textsize(action.upper(), font=self.font)
        pil_draw.text(((self.width - text_width) / 2, self.height // 16 + 50), action.upper(), (255, 255, 255),
                      font=self.font)
        image = np.array(pil_img)
        return image
